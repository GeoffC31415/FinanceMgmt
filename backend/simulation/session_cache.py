...
class FileBackedSessionCache(SessionCache):
    ...
    async def get(self, session_id: str) -> CachedSession | None:
        async with self._lock:
            entry = await self._read_index_entry(session_id)
            if entry is None:
                return None
            # Check TTL
            ...
            if not pkl_path.exists():
                ...
                await self._remove_index_entry(session_id)
                return None
            try:
    ...
    async def start_background_purge(self, interval_s: float = 5 * 60) -> None:
        """Start a background task that purges expired sessions periodically."""
        if self._running:
            return
        self._running = True
        self._purge_task = asyncio.create_task(
            self._purge_loop(interval_s)
        )

    async def _purge_loop(self, interval_s: float) -> None:
        while self._running:
            try:
                await asyncio.sleep(interval_s)
                removed = await self.purge_expired()
                if removed:
                    logger.info("Session cache: purged %d expired sessions", removed)
            except asyncio.CancelledError:
                break
            except Exception:
                logger.exception("Session cache purge error")
...
async def create_session_cache(
    *,
    ...
    if use_file_backed:
        ...
        await cache.start_background_purge(interval_s=5 * 60)
